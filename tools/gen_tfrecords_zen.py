...
# input_dir = "/home/paperspace/zen/6dof/models/research/kiseypointnet/tools/output_plane/02691156/"
...
def generate(files, output_dir, input_dir, gen_record, chunk_size=40):
    ...
    if os.path.isfile(gen_record):
        f_progress = open(gen_record, "r+")
    else:
        f_progress = open(gen_record, "w")
        f_progress.close()
        f_progress = open(gen_record, "r+")
                    ...
                    with tf.Session() as sess:
                        for file_name in files:
                            ...
                            while tf.gfile.Exists(indir + "%06d.txt" % count):
                                image0 = misc.imread(
                                    indir + "%06d.png" % (count * 2))
                                image1 = misc.imread(
                                    indir + "%06d.png" % (count * 2 + 1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--target', dest='target',
                        required=True,
                        help='target set: car, plane, chair, sofa')
    ...
    if args.target == 'car':
        output_dir = "/NAS/home/6dof/6dof_data/3dof/shapenet_car_light/cars_with_keypoints/"
        input_dir = "/NAS/home/6dof/6dof_data/3dof/shapenet_car_light/02958343/"
        gen_record = output_dir + "car_progress.txt"
    elif args.target == "plane":
        output_dir = "/NAS/home/6dof/6dof_data/3dof/shapenet_plane_light/planes_with_keypoints/"
        input_dir = "/NAS/home/6dof/6dof_data/3dof/shapenet_plane_light/02691156/"
        gen_record = output_dir + "plane_progress.txt"
    elif args.target == "chair":
        output_dir = "/NAS/home/6dof/6dof_data/3dof/shapenet_chair_light/chairs_with_keypoints/"
        input_dir = "/NAS/home/6dof/6dof_data/3dof/shapenet_chair_light/03001627/"
        gen_record = output_dir + "chair_progress.txt"
    elif args.target == "sofa":
        output_dir = "/NAS/home/6dof/6dof_data/2dof/sofas_with_keypoints/sofas_with_keypoints/"
        input_dir = "/NAS/home/6dof/6dof_data/2dof/sofas_with_keypoints/04256520/"
        gen_record = output_dir + "sofa_progress.txt"
    else:
        parser.print_help()
        exit(1)
